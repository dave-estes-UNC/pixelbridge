...
# Runs all of the experiments. This includes the recordings, the videos
# with normal playback, and the videos with rewind playbayback
#
# See runrecordings.sh, runvideos.sh, and runrewind.sh to run just
# a subset. These scripts take an additional parameter that specifies the
# file list.

if [ $# -ne 3 ]
then
	echo "Usage: runexperiments.sh <path-to-pixelbridge> <recordings-dir> <output-dirt>"
	exit 1
fi
...
#for x in `cat $REC_DIR/files`; do $PB --mode fb --start 200 --headless $REC_DIR/$x; done > $OUT_DIR/fb.log
#for x in `cat $REC_DIR/videos`; do $PB --mode fb --start 300 --frames 800 --headless $REC_DIR/$x; done > $OUT_DIR/video-fb.log
#for x in `cat $REC_DIR/videos`; do $PB --mode fb --start 300 --frames 1200 --rewind 800 200 --headless $REC_DIR/$x; done > $OUT_DIR/rewind-fb.log
#exit;
...
# Flat Tiled Mode
for x in `cat $REC_DIR/files`; do $PB --mode flat --start 200 --headless $REC_DIR/$x; done > $OUT_DIR/flat.log
for x in `cat $REC_DIR/videos`; do $PB --mode flat --start 300 --frames 800 --headless $REC_DIR/$x; done > $OUT_DIR/video-flat.log
for x in `cat $REC_DIR/videos`; do $PB --mode flat --start 300 --frames 1200 --rewind 800 200 --headless $REC_DIR/$x; done > $OUT_DIR/rewind-flat.log

# Cached Tiled Mode (8)
for x in `cat $REC_DIR/files`; do $PB --mode cache --tc 10000 --bits 8 --start 200 --headless $REC_DIR/$x; done > $OUT_DIR/cache8.log
for x in `cat $REC_DIR/videos`; do $PB --mode cache --tc 10000 --bits 8 --start 300 --frames 800 --headless $REC_DIR/$x; done > $OUT_DIR/video-cache8.log
for x in `cat $REC_DIR/videos`; do $PB --mode cache --tc 10000 --bits 8 --start 300 --frames 1200 --rewind 800 200 --headless $REC_DIR/$x; done > $OUT_DIR/rewind-cache8.log
...
for x in `cat $REC_DIR/files`; do $PB --mode count --start 200 --headless $REC_DIR/$x; done > $OUT_DIR/count.log
for x in `cat $REC_DIR/videos`; do $PB --mode count for--start 300 --frames 800 --headless $REC_DIR/$x; done > $OUT_DIR/video-count.log
for x in `cat $REC_DIR/videos`; do $PB --mode count –start 300 --frames 1200 --rewind 800 200 --headless $REC_DIR/$x; done > $OUT_DIR/rewind-count.log
