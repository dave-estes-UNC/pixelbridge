...
FfmpegPlayer::FfmpegPlayer(const char* fileName) : fileName_(fileName), width_(0), height_(0) {
	...
	// Open video file
	if (av_open_input_file(&pFormatCtx_, fileName_, NULL, 0, NULL) != 0)
		return; // Couldn't open file
	
	// Retrieve stream information
	if (av_find_stream_info(pFormatCtx_) < 0)
		return; // Couldn't find stream information
	
	// Dump information about file onto standard error
	dump_format(pFormatCtx_, 0, fileName_, false);
	
    // Find the first video stream
    videoStream_ = -1;
    for (int i = 0; i < pFormatCtx_->nb_streams; i++) {
        if (pFormatCtx_->streams[i]->codec->codec_type == CODEC_TYPE_VIDEO) {
            videoStream_ = i;
            break;
        }
	}
    ...
    // Open codec
    if (avcodec_open(pCodecCtx_, pCodec_) < 0)
...
FfmpegPlayer::~FfmpegPlayer() {
	...
	av_close_input_file(pFormatCtx_);
