...
void CachedTiler::UpdateDisplay(uint8_t* buffer, size_t width, size_t height)
{
	...
	for (int j_tile_map = 0; j_tile_map < tile_map_height_; j_tile_map++) {
		for (int i_tile_map = 0; i_tile_map < tile_map_width_; i_tile_map++) {
			...
			nddi::Pixel tile_pixels[tile_width_ * tile_height_];
			nddi::Pixel tile_pixels_sig_bits[tile_width_ * tile_height_];
...
#pragma omp ordered
			...
			tile.checksum = crc32(crc, (unsigned char*)tile_pixels_sig_bits, tile_width_ * tile_height_ * sizeof(nddi::Pixel));
			//tile.checksum = adler32(crc, (unsigned char*)tile_pixels_sig_bits, tile_width_ * tile_height_ * sizeof(nddi::Pixel));
				...
				// We didn't have room in the tile cache
					...
					// We couldn't re-use the previous zIndex, so we'll have to find another candidate in the cache
						...
						if (index >= 0) {
							...
							// ERROR
							std::cout << "ERROR: Couldn't find a candidate to eject from the cache." << std::endl;
						}
					}
				}
			}
	...
	if (!quiet_) {
		std::cout << "Cached Tiling Statistics:" << endl << "  unchanged: " << unchanged_tiles_ << " cache hits: " << cache_hits_ << " cache misses: " << cache_misses_ << " cache size: " << tile_cache_.size() << std::endl;
	}
}
 ...
 */
void CachedTiler::UpdateFrameVolume(nddi::Pixel* pixels, tile_t tile) {
